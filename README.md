...
Numbers below are from a full `python3 run_experiment.py` run on 2026-09-13, after the forecast bias and sigma were switched to an expanding window that only uses days before the one being scored (see `docs/decisions_log.md`, 2026-09-13).

- **Model comparison.** The Bayesian model clearly outperforms the Gaussian and KDE baselines on both proper scoring rules. Brier score 0.740 vs. 0.902 for both baselines, log loss 1.523 vs. 2.441 (Gaussian) and 2.383 (KDE). That is an 18% Brier skill score and a 38% log loss skill score over the Gaussian baseline. KDE vs. Gaussian is a wash.
- **Calibration.** The Bayesian model is well calibrated across its five probability buckets on out of sample data, for example predicted 0.249 vs. observed 0.246 in the middle bucket and 0.418 vs. 0.399 in the top bucket. Gaussian and KDE are overconfident at the top end (predicted 0.242 vs. observed 0.164). Full tables in `notebooks/06_Calibration_Analysis.ipynb`.
- **Backtest.** This tracks through into the backtest. Bayesian ended with the highest cumulative profit *and* the lowest VaR, Expected Shortfall, and max drawdown of the three models, on nearly identical trade counts (644 to 648). Bayesian profit 113.5 / VaR 0.100 / Expected Shortfall 0.160 / max drawdown 1.15 / Sharpe 0.129, vs. Gaussian 45.2 / 0.168 / 0.219 / 2.13 / 0.102 and KDE 45.5 / 0.166 / 0.216 / 1.77 / 0.102. Sharpe here is mean over standard deviation of per trade profit, not annualised. Better calibrated probabilities are translating into better risk adjusted outcomes, not just better scores in isolation.

## Known limitations

- **No real bid/ask spread, the biggest open limitation.** Spread is a flat assumed constant (`ASSUMED_SPREAD = 0.05` in `config/settings.py`, used by `pricing/edge.py:effective_edge()`), not real historical data, and this is a **permanent** decision, not a temporary gap. Polymarket's live order book API only covers currently open markets (every market here is already resolved). Options considered and ruled out are Dome API (real, but Polymarket acquired and shut it down in April 2026), PolymarketData.co (paid/tiered), Bitquery (wrong kind of data, trades, not order books, also paid), and pmxt (live only, no historical support). Full L2 order book history is expensive enough to store that every option either charges, expects self hosted chain indexing, or doesn't have the real bid/ask at all. See `docs/decisions_log.md`.
...
- **How in sample and out of sample apply here.** The climatological prior and the Gaussian and KDE fits use `IS_START` to `IS_END` (2000 to 2016) weather data only. Everything scored or traded lies in the OOS window from 2017 onward, and every Polymarket market falls inside it. The Bayesian forecast bias and sigma are estimated from an expanding window of paired forecast/actual days strictly before the scored day, with the first 30 days of the OOS period skipped for all three models so they are compared on the same days. What has not been done is a time split within the backtest window (early vs. late) to check that the edge is stable over time.
