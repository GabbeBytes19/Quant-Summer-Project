...
def synthetic_previous_df() -> pl.DataFrame:
    ...
    with a small warm bias (larger for longer lead times) plus a day to day error so the
    forecast error has a non zero spread, like a real forecast.
    """
    daily_max = [30.1, 31.4, 29.8, 32.0, 33.1, 30.5, 29.2, 31.8, 32.6, 30.9]
    bias_by_lead = [0.8, 1.0, 1.1, 1.2, 1.4]
    error_by_day = [0.3, -0.2, 0.1, -0.4, 0.2, 0.0, -0.1, 0.4, -0.3, 0.1]

    times, day1, day2, day3, day4, day5 = [], [], [], [], [], []
    columns = [day1, day2, day3, day4, day5]

    for d, (peak, day_error) in enumerate(zip(daily_max, error_by_day), start=1):
        for h in range(24):
            times.append(f"2024-06-{d:02d}T{h:02d}:00")
            hour_shape = -4 + 4 * (1 - abs(h - 14) / 14)
            for col, bias in zip(columns, bias_by_lead):
                col.append(round(peak + hour_shape + bias + day_error, 1))
